...
export const removeAddress = async (req = request, res = response) => {
  ...
  try {
    ...
    await Driver.findOneAndUpdate(
      { _id: idDriver },
      { $set: { order: order, status: "disponible", upsert: true } }
    );
...
export const finishTravelUser = async (req, res = response) => {
...
export const getUbicaciones = async (req, res = response) => {
  try {
    const addresses = await Address.find({
      $and: [{ _id: { $ne: req._id } }, { estado: true }],
    }).sort({ createdAt: "asc" });
...
export const getUbicacionesAutomatic = async (res = response) => {
  const idAdmin = "6439d6398dbdb6d5224e0bd6";
  const idOrderNull = [{ miId: "1" }];
  try {
    const data = await Address.find({
      $and: [{ _id: { $ne: idAdmin } }, { estado: true }],
    })
      .sort({ createdAt: "asc" })
      .limit(20);
