...
export const statusDriverArrived = async (req = request, res = response) => {
...
export const locationDriverUpdate = async (req = request, res = response) => {
  ...
  try {
    const driverLocation = await Address.findOneAndUpdate(
      { _id: idOrder },
...
export const statusUpdate = async (req = request, res = response) => {
  ...
  try {
    const driverStatus = await Driver.findByIdAndUpdate(
      { _id: _id },
      { $set: { order: order }, $inc: { viajes: viajes } },
      { upsert: true }
    );
...
export const finishTravel = async (req = request, res = response) => {
  ...
  try {
    const UserAddress = await Address.findOneAndUpdate(
      { idDriver: idDriver },
      { $set: { estado: true }, $unset: { idDriver: "" } }
    );
