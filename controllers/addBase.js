...
export const addBaseDriver = async (req = request, res = response) => {
  ...
  const existBase = await Base.find({
    $and: [{ base: base }, { zonaName: zona }],
  });
  ...
  const addDriver = await Base.findOneAndUpdate(
    { _id: new ObjectId(id) },
    { $addToSet: { idDriver: idDriver } },
    { new: true }
  );
...
export const addBaseAdmin = async (req = request, res = response) => {
  ...
  const existBase = await Base.findOne({
    $and: [{ base: base }, { zonaName: zona }],
  });
  ...
  if (existBase) {
    ...
    const registerOk = await Admin.findOneAndUpdate(
      { _id: idAdmin },
      { $set: { base: base } },
      { new: true }
    );
    ...
    await Zona.findOneAndUpdate(
      { nombre: zona },
      { $set: { basesId: new ObjectId(idBase) } },
      { new: true }
    );
    ...
    if (!registerOk) {
      return res
        .status(400)
        .json({ ok: false, msg: "Error al Guardar Numero de Base" });
    }
...
export const getDriversfromBase = async (req = request, res = response) => {
