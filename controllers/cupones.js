...
export const addPrice = async (req = request, res = response) => {
  ...
  try {
    ...
    if (existVaucher == false)
      return res.json({ msg: "Por favor ingrese un vaucher previamente" });

    const savePrice = await Usuario.findOneAndUpdate(
      { _id: idUser },
      { $set: { "cupon.1": costo } },
      { new: true }
    );
...
export const addVaucher = async (req = request, res = response) => {
  const idUser = req.params._id;
...
export const deletedVauchers = async (req = request, res = response) => {
  const idUser = req.params._id;
...
const comprobarVauchers = async (idUser) => {
  const vauchers = await Usuario.find(
    { _id: idUser },
    { role: 0, _id: 0, nombre: 0, password: 0, online: 0, __v: 0, email: 0 }
  );
